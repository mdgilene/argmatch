...
import argmatch.attributes as __unit__
...
class Attrs(MatcherTestCase):
    ...
    def test_match__both__mix(self):
        attrs = dict(foo=self.VALUE, bar=self.VALUE * 2, baz=self.VALUE * 2 + 1)
        foo_bar_baz = self.Object(**attrs)

        self.assert_match(foo_bar_baz, "bar", "baz", foo=self.VALUE)
        self.assert_match(foo_bar_baz, "baz", foo=self.VALUE, bar=self.VALUE * 2)
    ...
    def assert_match(self, value, *args, **kwargs):
        return super(Attrs, self).assert_match(__unit__.Attrs(*args, **kwargs), value)

    def assert_no_match(self, value, *args, **kwargs):
        return super(Attrs, self).assert_no_match(
            __unit__.Attrs(*args, **kwargs), value
        )
