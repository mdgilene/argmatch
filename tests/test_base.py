...
"""
import argmatch.base as __unit__
...
class BaseMatcherMetaclass(TestCase):
    ...
    def test_is_base_matcher_class_definition(self):
        ...
        is_bmcd = __unit__.BaseMatcherMetaclass._is_base_matcher_class_definition
    ...
    def test_list_magic_methods(self):
        ...
        self.assertListEqual([], lmm(Foo))

        class Bar(object):
            def method(self):
                pass

        self.assertListEqual([], lmm(Bar))
        ...
        self.assertListEqual(["init", "rdiv"], lmm(Baz))
        ...
        self.assertListEqual(["init"], lmm(Qux))
        ...
        self.assertListEqual(["bool", "nonzero"], lmm(Thud))
...
class Matcher(TestCase):
    ...
    def test_repr__argful_ctor__with_state_from_args(self):
        ...
        foo = "bar"
        self.assertEquals("<Custom(foo=%r)>" % (foo,), "%r" % Custom(foo="bar"))
...
class LogicalCombinators(MatcherTestCase):
    ...
    def test_and__idempotent(self):
        ...
        for s in test_strings:
            self.assertEquals(
                all_digits.match(s),
                all_digits_and_all_digits.match(s),
                msg="expected `%r` and `%r` to match %r equivalently"
    ...
    def test_and__regular(self):
        ...
        for s in test_strings:
            self.assertEquals(
                short_digits.match(s),
                short_and_digits.match(s),
                msg="expected `%r` and `%r` to match %r equivalently"
    ...
    def test_or__regular(self):
        ...
        for s in test_strings:
            self.assertEquals(
                long_or_has_digits.match(s),
                has_digits_or_long.match(s),
                msg="expected `%r` and `%r` to match %r equivalently"
    ...
    def test_xor__impossible(self):
        ...
        for s in test_strings:
            self.assertFalse(
                impossible.match(s), msg="%r matched an impossible matcher" % (s,)
            )
    ...
    def test_xor__as_and_not(self):
        ...
        for s in test_strings:
            ...
            self.assertEquals(
                only_some_digits.match(s),
                any_xor_all_digits.match(s),
                msg="expected `%r` and `%r` to match %r equivalently"
